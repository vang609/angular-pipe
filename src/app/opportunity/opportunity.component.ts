...
export class OpportunityComponent implements OnInit {
  ...
  nextPrev(next: boolean) { // If next step then pass true; If previous step then pass false;
    if (next) {
      ...
      this.stepperComponent.stepClick(this.stepperConst[this.currentStepperStep.name], next);
      this.formValid = false;
    } else {
      this.stepperComponent.stepClick(this.stepperConst[this.currentStepperStep.name - 2], next);
      this.formValid = true;
    }

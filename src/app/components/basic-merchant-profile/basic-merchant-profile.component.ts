...
export class BasicMerchantProfileComponent implements OnInit, OnDestroy, AfterContentChecked {
  state = [];
  ...
  maskFocus(e) {
    if (e.value === '') {
      e.input.nativeElement.selectionStart = 0;
      e.input.nativeElement.selectionEnd = 0;
    } else {
      const cursorPos = e.value.indexOf('_');
  ...
  maskKeyedIn(e: any, type: string) {
    if ((e.keyCode > 47 && e.keyCode < 58) || e.key === 'Backspace') {
    ...
    if (e.target.value.length < 12) {
      if ((e.keyCode > 47 && e.keyCode < 58) || e.key === 'Backspace') {
        ...
        if (e.key === 'Backspace') {
          ...
          if (type === 'ssn') {
            this.ssnVal.splice(pos, 1);
          }
          if (type === 'tin') {
            this.tinVal.splice(pos, 1);
          }
        }
      } else {
        if (type === 'ssn') {

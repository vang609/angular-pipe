...
import { AppShareDataService } from '../../shared/service/shareData.service ';
...
export class BasicMerchantProfileComponent implements OnInit, OnDestroy, AfterContentChecked, AfterViewInit {
  ...
  constructor(private stateService: StateService,
    ...
    private shareDataService: AppShareDataService,
    ...
    if (uVal.length < 10) {
      if ((e.keyCode > 47 && e.keyCode < 58) || e.keyCode === 8 || e.keyCode === 9 || (e.keyCode > 95 && e.keyCode < 106)) {
        ...
        if (e.keyCode === 8) {
          ...
          if (type === 'tin') {
            this.tinVal = this.tinVal.slice(0, inputVal.length);
          }
        }
      } else {
        if (type === 'ssn' && uVal.length < 9) {
          this.showInvalidSSNmessage = true;
          setTimeout(() => {
            this.showInvalidSSNmessage = false;
          }, 3000);
        }
        if (type === 'tin' && uVal.length < 9) {
  ...
  setFormValues(details: any): void {
    ...
    if (details.customers) {
      ...
      if (details.customers[0].addresses) {
        ...
        const address1 = street_address1 || '';
        const address2 = street_address2 || '';
        const address3 = street_address3 || '';
        const concatAddress = address1 + ', ' + address2 + ' ' + address3;

...
export class HttpInterceptorService implements HttpInterceptor {
    ...
    return next.handle(authReq).pipe(
      map((event: HttpEvent<any>) => {
        if (event instanceof HttpResponse) {
          if (event.body.statusInfo && event.body.statusInfo.length > 0) { // Specific for MRDS service
            let statusInfo = event.body.statusInfo;
            // console.log(statusInfo[0].code);
            if (statusInfo[0].code === '404') {
              statusInfo[0].code = 'MRDS-404';
            }
            Object.keys(this.errorMsg).map((key) => {
              if (key === statusInfo[0].code) {
                const message = new Message(key, this.errorMsg[key], '');
                this.store.dispatch(msgActions.setMessage({ message }));
                statusInfo[0].message = this.errorMsg[key];
                this.appShareDataService.messageMatrixSubject.next(statusInfo[0].message);
              }
            });
          } else { // For other than non-MRDS service
            if (event.body && event.body.validationRules) {
              if ( event.body.validationRules[0] ) {
                const validationObj = event.body.validationRules[0];
                Object.keys(this.errorMsg).map((key) => {
                  if (key ===  validationObj.code) {
                    const message = new Message(key, this.errorMsg[key], '');
                    this.store.dispatch(msgActions.setMessage({ message }));
                    validationObj.detail = this.errorMsg[key];
                    this.appShareDataService.messageMatrixSubject.next(validationObj.detail);
                  }
                });
              }
            }
      ...
      catchError(( httpErrorResponse: HttpErrorResponse) => {
        if ( httpErrorResponse.status.toString().startsWith('4')
              && !httpErrorResponse.error.customers.length
                && httpErrorResponse.statusText.toUpperCase() === 'NOT FOUND' ) {

        }
        return throwError(httpErrorResponse);
      }),
      finalize(() => this.appShareDataService.loadingSubject.next(false))
    );
  }
  ...
  //                 this.appShareDataService.messageMatrixSubject.next(statusMessage);
  //               }
  //             });
  //         }
  //       }

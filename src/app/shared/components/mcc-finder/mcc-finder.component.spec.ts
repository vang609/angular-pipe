...
describe('MccFinderComponent', () => {
  ...
  it('should call valuechange function if string length is greater than or equal to 3', () => {
    component.valuechange();
    spyOn(mccFinderService, 'getMCCIData').withArgs('test', 'N').and.returnValues(of(data.gridResp));
    expect(component.valuechange).toBeTruthy();
  });

  it('should call valuechange function if string length is less than 3', () => {
    component.srcString = 'hi';
    component.valuechange();
    expect(component.valuechange).toBeTruthy();
  });

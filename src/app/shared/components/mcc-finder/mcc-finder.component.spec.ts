...
describe('MccFinderComponent', () => {
  ...
  it('table should be filled', () => {
    component.grid = grid;
    component.mccSrc(data.mockup.mccSrcEvent, data.mockup.mccSrcEventTarget);
    expect(grid.expandRow).toHaveBeenCalled();
    expect(component.mccData.length).toBeGreaterThan(0);
  });

  it('Notification message should be shown', () => {
    component.srcString = 'He';
    component.srcStringMinLength = 3;
    // spyOn(notificationService, 'show').and.();
    component.mccSrc(data.mockup.mccSrcEvent, data.mockup.mccSrcEventTarget);
